...
void Signup::UpdateInputs(HWND hDlg, int enabled)
{
    ...
    EnableWindow(GetDlgItem(hDlg, IDC_SERVERNUM), enabled);
}
...
{
    HWND hUsername, hPasswd1, hPasswd2, hEmail, hServer;
    ...
    case WM_INITDIALOG:
        ...
        hServer = GetDlgItem(hDlg, IDC_NEW_EMAIL);
        ...
        {
            SetFocus(hEmail);
            CheckDlgButton(hDlg, IDC_SERVERNUM, TRUE);
            return 0;
        }
        break;
    case WM_COMMAND:
        ...
        case IDC_SIGNUP_OK:
        {
            ...
            if (!SendDlgItemMessage(hDlg, IDC_SERVERNUM, BM_GETCHECK, 0, 0))
            {
                server = 102;
            }
